// Путь: src/components/ui/feed-info/feed-info.tsx
import React, { FC, memo } from 'react';
import styles from './feed-info.module.css';
import glow from '../order-details/order-details.module.css'; // .title содержит text-shadow
    ...
    <ul className={`pt-6 ${styles.list}`}>
      {orders.map((item, index) => (
        <li
          ...
          // твой исходный цвет — возвращаю как был
...
const Column: FC<TColumnProps> = ({ title, content }) => (
  <>
    ...
    {/* навешиваем готовый "неон" из order-details */}
    <p
      className={`text text_type_digits-large ${styles.content} ${glow.title}`}
    >
      {typeof content === 'number' ? content.toLocaleString('ru-RU') : content}
    </p>
  </>
);
